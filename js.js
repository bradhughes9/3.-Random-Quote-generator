 const randomQuote = () => {
    ...
    //Get the key and leave the values. Put the keys into an array
    const quotesKeys = Object.keys(quotes);
    const quotesQuote = Object.values(quotes);
    //Console.log to make sure everything is displayed correctly
    //console.log(quotesKeys);

    //Now randomly select a key/author with the length of the object array
    let randomIndex = Math.floor(Math.random() * quotesKeys.length);

    document.getElementById('quote').innerHTML = `${quotesQuote[randomIndex]}`;
    document.getElementById('author').innerHTML = `${quotesKeys[randomIndex]}`;
    //Console.log to make sure everything is displayed correctly here as well
    //console.log(quotesQuote[randomIndex]);
    //console.log(quotesKeys[randomIndex]);
